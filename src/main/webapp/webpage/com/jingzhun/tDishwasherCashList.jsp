...
<body>
	<div class="panel-body" style="padding-top: 0px; padding-bottom: 0px;">
		<!-- toolbar -->
		<div id="toolbar">
			<button onclick="add('录入','tDishwasherCashController.do?goAdd','tDishwasherCashList',600,400)" id="btn_add" class="btn btn-primary btn-sm">
				<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> 录入
			</button>
			<button onclick="update('编辑','tDishwasherCashController.do?goUpdate','tDishwasherCashList',600,400)" id="btn_edit" class="btn btn-success btn-sm">
				<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> 编辑
			</button>
			<button onclick="deleteALLSelect('批量删除','tDishwasherCashController.do?doBatchDel','tDishwasherCashList',600,400)" id="btn_delete" class="btn btn-danger btn-sm">
				<span class="glyphicon glyphicon-remove" aria-hidden="true"></span> 批量删除
			</button>
			<button onclick="detail('查看','tDishwasherCashController.do?goUpdate','tDishwasherCashList',600,400)" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-search" aria-hidden="true"></span> 查看
			</button>
			<button onclick="ImportXls('导入','','tDishwasherCashList',600,400)" id="btn_download" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-cloud-download" aria-hidden="true"></span> 导入
			</button>
			<button onclick="ExportXls('导出','','tDishwasherCashList',600,400)" id="btn_upload" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-cloud-upload" aria-hidden="true"></span> 导出
			</button>
			<button onclick="ExportXlsByT('模板下载','','tDishwasherCashList',600,400)" id="btn_upload" class="btn btn-info btn-sm">
		...
		<div class="table-responsive">
			...
			<table id="tDishwasherCashList"></table>
		</div>
	</div>
	<script type="text/javascript">
		...
		var tDishwasherCashListdictsData = {};
		$(function() {
			var promiseArr = [];
			...
			$('#tDishwasherCashList').on(
					'check.bs.table uncheck.bs.table load-success.bs.table '
							+ 'check-all.bs.table uncheck-all.bs.table',
					function() {
						$('#btn_delete').prop('disabled',!$('#tDishwasherCashList').bootstrapTable('getSelections').length);
						$('#btn_edit').prop('disabled',$('#tDishwasherCashList').bootstrapTable('getSelections').length != 1);
					});
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$('#tDishwasherCashList').bootstrapTable({
									url : 'tDishwasherCashController.do?datagrid', //请求后台的URL（*）
									method : 'get', //请求方式（*）
									...
									columns : [
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var pageSize = $('#tDishwasherCashList')
															.bootstrapTable('getOptions').pageSize;
													var pageNumber = $('#tDishwasherCashList').bootstrapTable('getOptions').pageNumber;
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var href = '';
													href += "<a href='javascript:void(0);'  class='ace_button'  onclick=delObj('tDishwasherCashController.do?doDel&id="
															+ row.id
			...
			oTableInit.queryParams = function(params) {
				var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
					...
					field : 'id,cashDate,cashScore,cashState,cashAccountId,cashUserId,'
				};
		...
		function reloadTable() {
			$('#tDishwasherCashList').bootstrapTable('refresh');
		}
	...
	<script type="text/javascript">
		...
		//导入
		function ImportXls() {
			openuploadwin('Excel导入', 'tDishwasherCashController.do?upload',"tDishwasherCashList");
		}

		//导出
		function ExportXls() {
			JeecgExcelExport("tDishwasherCashController.do?exportXls","tDishwasherCashList");
		}

		//模板下载
		function ExportXlsByT() {
			JeecgExcelExport("tDishwasherCashController.do?exportXlsByT","tDishwasherCashList");
		}
		...
		function listDictFormat(value,code,table){
			...
			var dicts = tDishwasherCashListdictsData[dictKey];
		...
		function loadSearchFormDicts(obj,table,code,type,name){
			...
			var arr = tDishwasherCashListdictsData[dictKey];
