...
<body>
	<div class="panel-body" style="padding-top: 0px; padding-bottom: 0px;">
		<!-- toolbar -->
		<div id="toolbar">
			<button onclick="add('录入','tDishwasherLogController.do?goAdd','tDishwasherLogList',600,400)" id="btn_add" class="btn btn-primary btn-sm">
				<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> 录入
			</button>
			<button onclick="update('编辑','tDishwasherLogController.do?goUpdate','tDishwasherLogList',600,400)" id="btn_edit" class="btn btn-success btn-sm">
				<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> 编辑
			</button>
			<button onclick="deleteALLSelect('批量删除','tDishwasherLogController.do?doBatchDel','tDishwasherLogList',600,400)" id="btn_delete" class="btn btn-danger btn-sm">
				<span class="glyphicon glyphicon-remove" aria-hidden="true"></span> 批量删除
			</button>
			<button onclick="detail('查看','tDishwasherLogController.do?goUpdate','tDishwasherLogList',600,400)" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-search" aria-hidden="true"></span> 查看
			</button>
			<button onclick="ImportXls('导入','','tDishwasherLogList',600,400)" id="btn_download" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-cloud-download" aria-hidden="true"></span> 导入
			</button>
			<button onclick="ExportXls('导出','','tDishwasherLogList',600,400)" id="btn_upload" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-cloud-upload" aria-hidden="true"></span> 导出
			</button>
			<button onclick="ExportXlsByT('模板下载','','tDishwasherLogList',600,400)" id="btn_upload" class="btn btn-info btn-sm">
		...
		<div class="table-responsive">
			...
			<table id="tDishwasherLogList"></table>
		</div>
	</div>
	<script type="text/javascript">
		...
		var tDishwasherLogListdictsData = {};
		$(function() {
			var promiseArr = [];
			...
			$('#tDishwasherLogList').on(
					'check.bs.table uncheck.bs.table load-success.bs.table '
							+ 'check-all.bs.table uncheck-all.bs.table',
					function() {
						$('#btn_delete').prop('disabled',!$('#tDishwasherLogList').bootstrapTable('getSelections').length);
						$('#btn_edit').prop('disabled',$('#tDishwasherLogList').bootstrapTable('getSelections').length != 1);
					});
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$('#tDishwasherLogList').bootstrapTable({
									url : 'tDishwasherLogController.do?datagrid', //请求后台的URL（*）
									method : 'get', //请求方式（*）
									...
									columns : [
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var pageSize = $('#tDishwasherLogList')
															.bootstrapTable('getOptions').pageSize;
													var pageNumber = $('#tDishwasherLogList').bootstrapTable('getOptions').pageNumber;
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var href = '';
													href += "<a href='javascript:void(0);'  class='ace_button'  onclick=delObj('tDishwasherLogController.do?doDel&id="
															+ row.id
		...
		function reloadTable() {
			$('#tDishwasherLogList').bootstrapTable('refresh');
		}
	...
	<script type="text/javascript">
		...
		//导入
		function ImportXls() {
			openuploadwin('Excel导入', 'tDishwasherLogController.do?upload',"tDishwasherLogList");
		}

		//导出
		function ExportXls() {
			JeecgExcelExport("tDishwasherLogController.do?exportXls","tDishwasherLogList");
		}

		//模板下载
		function ExportXlsByT() {
			JeecgExcelExport("tDishwasherLogController.do?exportXlsByT","tDishwasherLogList");
		}
		...
		function listDictFormat(value,code,table){
			...
			var dicts = tDishwasherLogListdictsData[dictKey];
		...
		function loadSearchFormDicts(obj,table,code,type,name){
			...
			var arr = tDishwasherLogListdictsData[dictKey];
